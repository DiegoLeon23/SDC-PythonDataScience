# -*- coding: utf-8 -*-
"""
Created on Sat Feb 20 09:17:06 2021
...
def convert_numeric_object(df,list_text):
    ...
    #Entrenamos y transformamos las diferentes variables que esten en la "list_text",que se guardara en nuestro "defaultdict(d)""
    fit=df.apply(lambda x: d[x.name].fit_transform(x) if x.name in list_text else x)

    # Usando el diccionario para etiquetar datos futuros
    df=df.apply(lambda x: d[x.name].transform(x) if x.name in list_text else x)
    #tspr existen nan
    return df, d
